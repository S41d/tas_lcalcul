...
let rec show_pterm_simp t =
  ...
  | List l -> sprintf "[%s]" (String.concat "; " (List.map show_pterm_simp l))
...
let rec alphaconv term =
  let rec aux cur_var repl_var = function
    ...
    | List terms -> List (List.map (aux cur_var repl_var) terms)
  ...
  | List terms -> List (List.map alphaconv terms)
...
let rec subst id to_put = function
  ...
  | List terms -> List (List.map (fun x -> subst id to_put x) terms)
...
let rec ltr_ctb_step t =
  ...
  in
  match t with
  | Abs (x, t) -> map (fun step_t -> Abs (x, step_t)) (ltr_ctb_step t)
  | App (Abs (x, t), targ) when is_value targ -> Some (subst x targ t)
  | App (tfun, targ) -> map (fun (l, r) -> App (l, r)) (step_lr tfun targ)
  | Var _ -> None

  | Nat _ -> None
  | Add (t1, t2) -> map (fun (l, r) -> Add (l, r)) (step_lr t1 t2)
  | Sub (t1, t2) -> map (fun (l, r) -> Sub (l, r)) (step_lr t1 t2)
  ...
  | IfZero (cond, tthen, telse) ->
    (match ltr_ctb_step cond with
     ...
     | None -> match cond with
       | Nat 0 -> ltr_ctb_step tthen
       | Nat _ -> ltr_ctb_step telse
  ...
  | IfEmpty (cond, tthen, telse) ->
    (match ltr_ctb_step cond with
     ...
     | None -> match cond with
       | List [] -> ltr_ctb_step tthen
       | List _ -> ltr_ctb_step telse
       | _ -> failwith "Type error, expected list in IfEmpty condition")

  | Fix t -> map (fun t -> Fix t) (ltr_ctb_step t)

  | Let (id, te, tin) ->
    (match ltr_ctb_step te with
     | Some t -> Some (Let (id, t, tin))
     | None -> map (fun t -> Let (id, te, t)) (ltr_ctb_step tin))
;;
